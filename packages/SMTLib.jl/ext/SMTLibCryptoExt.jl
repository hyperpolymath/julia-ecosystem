...
#
# SMTLib.jl Crypto Extension
#
# Cryptographic Accelerator acceleration for SMT solving operations.

module SMTLibCryptoExt
...
function __init__()
    register_operation!(CryptoAccelBackend, :solve)
    register_operation!(CryptoAccelBackend, :check_sat)
    register_operation!(CryptoAccelBackend, :model_eval)
    register_operation!(CryptoAccelBackend, :simplify)
end
...
function SMTLib.backend_coprocessor_solve(b::CryptoAccelBackend,
    ...
    try
        ...
        @warn "SMTLibCryptoExt: solve failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_check_sat(b::CryptoAccelBackend,
                                               clauses::AbstractVector,
                                               n_vars::Int)
    n_clauses = length(clauses)
    ...
    try
        ...
        _record_diagnostic!("cryptoaccel", "runtime_errors")
        @warn "SMTLibCryptoExt: check_sat failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_model_eval(b::CryptoAccelBackend,
    ...
    try
        ...
        _record_diagnostic!("cryptoaccel", "runtime_errors")
        @warn "SMTLibCryptoExt: model_eval failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_simplify(b::CryptoAccelBackend,
    ...
    try
        ...
        _record_diagnostic!("cryptoaccel", "runtime_errors")
        @warn "SMTLibCryptoExt: simplify failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_interpolate(b::CryptoAccelBackend, args...)
    return nothing
end

end # module SMTLibCryptoExt

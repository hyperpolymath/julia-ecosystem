...
Cliodynamics.jl provides plot recipes via a package extension that loads
automatically when `+Plots.jl+` (or `+RecipesBase+`) is available.
...
Scatter plot with phases color-coded: green (Expansion), yellow
(Stagflation), red (Crisis), blue (Depression).

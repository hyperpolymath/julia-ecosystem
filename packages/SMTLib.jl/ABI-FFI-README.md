...
│       └── include/
│           └── smtlib.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── smtlib.h       # Generated from Idris2 ABI
│
...
int main() {
    void* handle = smtlib_init();
    if (!handle) return 1;

    int result = smtlib_process(handle, 42);
    if (result != 0) {
        const char* err = smtlib_last_error();
...
extern "C" {
    ...
    fn smtlib_free(handle: *mut std::ffi::c_void);
    fn smtlib_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = smtlib_init();
        assert!(!handle.is_null());

        let result = smtlib_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:smtlib_init, libsmtlib), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:smtlib_process, libsmtlib), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:smtlib_free, libsmtlib), Cvoid, (Ptr{Cvoid},), handle)
end

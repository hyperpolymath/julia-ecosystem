...
#
# KnotTheory.jl Metal Extension
# GPU-accelerated kernels for knot polynomial computation on Apple Silicon
# via Metal.jl and KernelAbstractions.jl. Mirrors the CUDA extension with
# Metal-specific array types and backend.

module KnotTheoryMetalExt
...
# ============================================================================
#
# Identical algorithm to the CUDA kernel; KernelAbstractions provides
# portability. Each thread handles one of 2^n Kauffman bracket states.
...
# ============================================================================
# Host-side: Jones Polynomial via Metal GPU
# ============================================================================

function backend_jones_invariant(::GPUBackend, pd::PlanarDiagram, wr::Int)
    # Check that Metal is actually the active backend
    # (GPUBackend is shared; only proceed if Metal devices are available)
    ...
    if n > 28
        @warn "Metal bracket limited to 28 crossings (got $n); falling back to CPU"
        return nothing
    end
    ...
    crossing_arcs_d = MtlArray(crossing_arcs_h)
    crossing_signs_d = MtlArray(crossing_signs_h)
    pair_a_d = isempty(pair_a_h) ? Metal.zeros(Int32, 1) : MtlArray(pair_a_h)
    pair_b_d = isempty(pair_b_h) ? Metal.zeros(Int32, 1) : MtlArray(pair_b_h)
    coeffs_d = Metal.zeros(Int32, coeff_len)
...
function gpu_poly_lu_det(PM::Matrix{Vector{Int}}, n::Int)
    ...
    # Work on CPU with the prepared data (hybrid approach:
    # GPU handles the bracket kernel; LU uses CPU with GPU-friendly layout)
    mat = poly_data_h
    sign = 1

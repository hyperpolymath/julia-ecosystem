...
#
# SMTLib.jl DSP Extension
#
# Digital Signal Processing acceleration for SMT solving operations.
...
function __init__()
    register_operation!(DSPBackend, :solve)
    register_operation!(DSPBackend, :check_sat)
    register_operation!(DSPBackend, :model_eval)
    register_operation!(DSPBackend, :simplify)
end
...
function SMTLib.backend_coprocessor_solve(b::DSPBackend,
    ...
    try
        ...
        @warn "SMTLibDSPExt: solve failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_check_sat(b::DSPBackend,
                                               clauses::AbstractVector,
                                               n_vars::Int)
    n_clauses = length(clauses)
    ...
    try
        ...
        _record_diagnostic!("dsp", "runtime_errors")
        @warn "SMTLibDSPExt: check_sat failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_model_eval(b::DSPBackend,
    ...
    try
        ...
        _record_diagnostic!("dsp", "runtime_errors")
        @warn "SMTLibDSPExt: model_eval failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_simplify(b::DSPBackend,
    ...
    try
        ...
        _record_diagnostic!("dsp", "runtime_errors")
        @warn "SMTLibDSPExt: simplify failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_interpolate(b::DSPBackend, args...)
    return nothing
end

end # module SMTLibDSPExt

...
#
# ZeroProb.jl ROCm Extension
#
# GPU-accelerated kernels for zero-probability measure computation on AMD GPUs.
# Uses KernelAbstractions.jl for portable kernel definitions and AMDGPU.jl for
# device management and memory transfers.
#
# Provides the same five GPU operations as the CUDA and Metal extensions:
...
# ============================================================================
# Kernel Definitions (KernelAbstractions.jl — identical to CUDA/Metal)
# ============================================================================
...
function _simpson_weights_roc(n_points::Int)
...
function _trapezoidal_weights_roc(n_points::Int)
    w = ones(Float64, n_points + 1)
    w[1] = 0.5
    w[end] = 0.5
    ROCArray(w)
...
function _eval_pdf_on_roc(dist::Distribution, xs::Vector{Float64})
    ROCArray(pdf.(Ref(dist), xs))
end

# KernelAbstractions backend for ROCm — obtain via get_backend on a device array
ROCmBackendKA() = KernelAbstractions.get_backend(ROCArray(zeros(Float64, 1)))

# ============================================================================
# Backend Hook: backend_sampling — Hausdorff Dimension (box-counting)
# ============================================================================

function ZeroProb.backend_sampling(backend::AcceleratorGate.ROCmBackend,
    ...
    try
        ...
        samples_gpu    = ROCArray(samples_cpu)
        membership_gpu = ROCArray(membership_cpu)
        scales_gpu     = ROCArray(scales_cpu)
        occupied_gpu   = AMDGPU.zeros(Int32, n_slots, n_scales)
        counts_gpu     = AMDGPU.zeros(Int32, n_scales)
        ...
        AcceleratorGate._record_diagnostic!("rocm", "runtime_errors")
        @warn "ZeroProbROCmExt: box-counting kernel failed, falling back" exception=e maxlog=1
        return nothing
    end
end
...
function ZeroProb.backend_sampling(backend::AcceleratorGate.ROCmBackend,
    ...
    try
        ...
        samples_gpu      = ROCArray(samples_cpu)
        logpdf_plus_gpu  = ROCArray(logpdf_plus_cpu)
        logpdf_minus_gpu = ROCArray(logpdf_minus_cpu)
        scores_sq_gpu    = AMDGPU.zeros(Float64, n_samples)
        ...
        AcceleratorGate._record_diagnostic!("rocm", "runtime_errors")
        @warn "ZeroProbROCmExt: Fisher information kernel failed, falling back" exception=e maxlog=1
        return nothing
    end
end
...
function ZeroProb.backend_log_likelihood(backend::AcceleratorGate.ROCmBackend,
    ...
    try
        ...
        P_gpu = ROCArray(P_cpu)
        Q_gpu = ROCArray(Q_cpu)
        weights_gpu = _trapezoidal_weights_roc(n_points)
        result_gpu  = AMDGPU.zeros(Float64, n_points + 1)
        ...
        AcceleratorGate._record_diagnostic!("rocm", "runtime_errors")
        @warn "ZeroProbROCmExt: KL divergence kernel failed, falling back" exception=e maxlog=1
        return nothing
    end
end
...
function ZeroProb.backend_marginalize(backend::AcceleratorGate.ROCmBackend,
                                      P::Distribution, Q::Distribution,
                                      n_points::Int)
    try
        ...
        P_gpu = _eval_pdf_on_roc(P, xs)
        Q_gpu = _eval_pdf_on_roc(Q, xs)
        weights_gpu = _simpson_weights_roc(n_points)
        result_gpu  = AMDGPU.zeros(Float64, n_points + 1)
        ...
        AcceleratorGate._record_diagnostic!("rocm", "runtime_errors")
        @warn "ZeroProbROCmExt: TV distance kernel failed, falling back" exception=e maxlog=1
        return nothing
    end
end
...
function ZeroProb.backend_marginalize(backend::AcceleratorGate.ROCmBackend,
    ...
    try
        ...
        pdf_gpu  = ROCArray(pdf_vals_cpu)
        cond_gpu = ROCArray(cond_vals_cpu)
        weights_gpu = _simpson_weights_roc(n_points)
        result_gpu  = AMDGPU.zeros(Float64, n_points + 1)
        ...
        AcceleratorGate._record_diagnostic!("rocm", "runtime_errors")
        @warn "ZeroProbROCmExt: conditional density kernel failed, falling back" exception=e maxlog=1
        return nothing
    end
end

end  # module ZeroProbROCmExt

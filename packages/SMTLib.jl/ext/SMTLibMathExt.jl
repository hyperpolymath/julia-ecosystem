...
#
# SMTLib.jl Math Extension
#
# Mathematical Accelerator acceleration for SMT solving operations.

module SMTLibMathExt
...
function __init__()
    register_operation!(MathAccelBackend, :solve)
    register_operation!(MathAccelBackend, :check_sat)
    register_operation!(MathAccelBackend, :model_eval)
    register_operation!(MathAccelBackend, :simplify)
end
...
function SMTLib.backend_coprocessor_solve(b::MathAccelBackend,
    ...
    try
        ...
        @warn "SMTLibMathExt: solve failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_check_sat(b::MathAccelBackend,
                                               clauses::AbstractVector,
                                               n_vars::Int)
    n_clauses = length(clauses)
    ...
    try
        ...
        _record_diagnostic!("mathaccel", "runtime_errors")
        @warn "SMTLibMathExt: check_sat failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_model_eval(b::MathAccelBackend,
    ...
    try
        ...
        _record_diagnostic!("mathaccel", "runtime_errors")
        @warn "SMTLibMathExt: model_eval failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_simplify(b::MathAccelBackend,
    ...
    try
        ...
        _record_diagnostic!("mathaccel", "runtime_errors")
        @warn "SMTLibMathExt: simplify failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_interpolate(b::MathAccelBackend, args...)
    return nothing
end

end # module SMTLibMathExt

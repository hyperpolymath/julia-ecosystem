...
#
# SMTLib.jl PPU Extension
#
# Physics Processing Unit acceleration for SMT solving operations.
...
function __init__()
    register_operation!(PPUBackend, :solve)
    register_operation!(PPUBackend, :check_sat)
    register_operation!(PPUBackend, :model_eval)
    register_operation!(PPUBackend, :simplify)
end
...
function SMTLib.backend_coprocessor_solve(b::PPUBackend,
    ...
    try
        ...
        @warn "SMTLibPPUExt: solve failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_check_sat(b::PPUBackend,
                                               clauses::AbstractVector,
                                               n_vars::Int)
    n_clauses = length(clauses)
    ...
    try
        ...
        _record_diagnostic!("ppu", "runtime_errors")
        @warn "SMTLibPPUExt: check_sat failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_model_eval(b::PPUBackend,
    ...
    try
        ...
        _record_diagnostic!("ppu", "runtime_errors")
        @warn "SMTLibPPUExt: model_eval failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_simplify(b::PPUBackend,
    ...
    try
        ...
        _record_diagnostic!("ppu", "runtime_errors")
        @warn "SMTLibPPUExt: simplify failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
end
...
function SMTLib.backend_coprocessor_interpolate(b::PPUBackend, args...)
    return nothing
end

end # module SMTLibPPUExt

...
#
# CladisticsMetalExt — Metal GPU kernels for Cladistics.jl
# Accelerates distance matrix, parsimony scoring, and bootstrap resampling
# on Apple Silicon GPUs via KernelAbstractions.jl + Metal.jl.

module CladisticsMetalExt

using Cladistics
using Metal
using Metal: MtlArray, MtlMatrix, MtlVector
using KernelAbstractions
using KernelAbstractions: @kernel, @index, @Const
using AcceleratorGate: MetalBackend, JuliaBackend, _record_diagnostic!

# ============================================================================
# Character Encoding (shared logic with CUDA ext)
# ============================================================================
...
function Cladistics.backend_distance_matrix(::MetalBackend, sequences::Vector{String}, method::Symbol)
    n = length(sequences)
    n < 16 && return nothing
    ...
    d_seqs = MtlArray(encoded)
    d_dmat = Metal.zeros(Float64, n, n)

    kernel = distance_kernel!(KernelAbstractions.MetalDevice(), (16, 16))
...
function Cladistics.backend_parsimony_score(::MetalBackend, tree::Cladistics.PhylogeneticTree,
    ...
    d_left = MtlArray(left_child)
    d_right = MtlArray(right_child)
    d_taxon = MtlArray(taxon_idx)
    d_chars = MtlArray(encoded_chars)
    d_scores = Metal.zeros(Int32, n_sites)
    d_scratch = Metal.zeros(UInt32, Int(n_nodes), n_sites)

    kernel = parsimony_kernel_with_scratch!(KernelAbstractions.MetalDevice(), 256)
...
function Cladistics.backend_bootstrap_replicate(::MetalBackend, sequences::Vector{String},
    ...
    d_col_indices = MtlArray(col_indices)

    d_resampled = Metal.zeros(UInt8, seq_len, n, replicates)

    kernel = bootstrap_resample_kernel!(KernelAbstractions.MetalDevice(), (16, 16))
    ...
    for rep in 1:replicates
        Metal.fill!(d_dmat, 0.0)
        d_rep_seqs = MtlArray(Array(d_resampled[:, :, rep]))

        dist_kernel = distance_kernel!(KernelAbstractions.MetalDevice(), (16, 16))
...
function Cladistics.backend_neighbor_join(::MetalBackend, dmat::Matrix{Float64}, taxa_names)
    # NJ is inherently sequential; GPU acceleration of Q-matrix only worthwhile
    # for very large n. Fall back to CPU.
    return nothing
end

function Cladistics.backend_tree_search(::MetalBackend, args...)
    return nothing
end

end # module CladisticsMetalExt

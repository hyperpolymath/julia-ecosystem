...
@testset "NPU strict mode compile gate" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test occursin("AXIOM_NPU_REQUIRED", sprint(showerror, err))
    end
end

@testset "NPU strict mode runtime hook gate" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test occursin("AXIOM_NPU_REQUIRED", msg)
        @test occursin("strict mode enabled", msg)
    end
end

# Install minimal NPU hook overrides to demonstrate strict-mode success path.
@eval begin
...
@testset "NPU strict mode with hook overrides" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test npu["required"] == true
        @test npu["hook_overrides"]["backend_coprocessor_matmul"] == true
        @test npu["hook_overrides"]["backend_coprocessor_relu"] == true
        @test npu["hook_overrides"]["backend_coprocessor_softmax"] == true
    end
end

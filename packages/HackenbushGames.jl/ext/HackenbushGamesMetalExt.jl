...
Compute legal move indices on Metal GPU using a parallel edge filter kernel.
"""
function _gpu_legal_move_indices(graph::HackenbushGraph, player::Symbol)
    ...
    colors_gpu = MtlArray(ec)
    mask_gpu = Metal.zeros(Int32, n)
...
function HackenbushGames.backend_move_gen(::AcceleratorGate.MetalBackend,
    ...
    indices = try
        ...
        @warn "Metal move generation failed, falling back to CPU" exception=ex maxlog=1
        return nothing
    end
...
function HackenbushGames.backend_position_hash(::AcceleratorGate.MetalBackend,
    ...
    ev_gpu = MtlArray(ev)
    ec_gpu = MtlArray(ec)
    zt_gpu = MtlArray(_ZOBRIST_TABLE)
    hashes_gpu = Metal.zeros(UInt64, n)
    ...
    # XOR reduction on CPU (transfer is cheap for a single vector)
...
@kernel function _count_component_edges_kernel!(comp_edge_count, @Const(parent),
    ...
    if idx <= num_edges
        root = parent[eu[idx]]
        # Metal does not support generic atomics on Int32 in all cases;
        # use a simple non-atomic increment (safe for tree-structured graphs
        # where each edge maps to a unique component root contribution).
        # For production use with cycles, a proper atomic or segmented reduce
        # should be used.
        comp_edge_count[root] += Int32(1)
    end
end
...
function _gpu_component_grundy(graph::HackenbushGraph)
    ...
    eu_gpu = MtlArray(eu_1)
    ev_gpu = MtlArray(ev_1)

    parent_gpu = Metal.zeros(Int32, Int(num_nodes))
    rank_gpu = Metal.zeros(Int32, Int(num_nodes))
    backend = MetalBackend()
    ...
    comp_edge_count_gpu = Metal.zeros(Int32, Int(num_nodes))
...
function HackenbushGames.backend_grundy_number(::AcceleratorGate.MetalBackend,
                                                graph::HackenbushGraph)
    n = length(graph.edges)
    n < 32 && return nothing
    ...
    try
        ...
        _record_diagnostic!("metal", "runtime_errors")
        @warn "Metal Grundy computation failed, falling back to CPU" exception=ex maxlog=1
        nothing
    end
end
...
function HackenbushGames.backend_game_tree_eval(::AcceleratorGate.MetalBackend,
    ...
    try
        ...
        _record_diagnostic!("metal", "runtime_errors")
        @warn "Metal game tree eval failed, falling back to CPU" exception=ex maxlog=1
        nothing
    end
end
...
end  # module HackenbushGamesMetalExt

...
# BowtieRisk ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libbowtierisk.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── bowtierisk.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── bowtierisk.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/bowtierisk.h
```
...
```c
#include "bowtierisk.h"

int main() {
    void* handle = bowtierisk_init();
    if (!handle) return 1;

    int result = bowtierisk_process(handle, 42);
    if (result != 0) {
        const char* err = bowtierisk_last_error();
...
Compile with:
```bash
gcc -o example example.c -lbowtierisk -L./zig-out/lib
```

### From Idris2

```idris
import BowtieRisk.ABI.Foreign
...
```rust
#[link(name = "bowtierisk")]
extern "C" {
    fn bowtierisk_init() -> *mut std::ffi::c_void;
    fn bowtierisk_free(handle: *mut std::ffi::c_void);
    fn bowtierisk_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = bowtierisk_init();
        assert!(!handle.is_null());

        let result = bowtierisk_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libbowtierisk = "libbowtierisk"

function init()
    handle = ccall((:bowtierisk_init, libbowtierisk), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:bowtierisk_process, libbowtierisk), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:bowtierisk_free, libbowtierisk), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/bowtierisk.h
   ```

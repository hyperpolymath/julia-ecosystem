...
The Malthusian logistic growth model describes population dynamics
constrained by carrying capacity:
...
where: - `+N+` = population size - `+r+` = intrinsic growth rate - `+K+`
= carrying capacity (resource-limited maximum)
...
* *State capacity* (`+S+`): Revenue from taxation, eroded by elite
demands

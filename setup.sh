...
# setup.sh — Universal setup script for julia-ecosystem
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/julia-ecosystem/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== julia-ecosystem Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/julia-ecosystem.git"
        info "Then: cd julia-ecosystem && ./setup.sh"
        exit 1
    fi

...
#
# ProvenCrypto Metal Extension
# Real GPU kernel implementations for Apple Silicon (M1/M2/M3/M4) using
# KernelAbstractions.jl for portable kernels and Metal.jl for GPU arrays.
...
ProvenCrypto.metal_available() = Metal.functional()

function ProvenCrypto.create_metal_backend()
    device_id = 0
    gen = ProvenCrypto.detect_apple_silicon_generation()
    has_ne = gen >= 2  # Neural Engine improved in M2+
    return ProvenCrypto.MetalBackend(device_id, has_ne, gen)
end

# ============================================================================
# KernelAbstractions GPU Kernels (portable -- shared algorithm with CUDA)
# ============================================================================

# --- Forward NTT Butterfly (Cooley-Tukey) ---
...
# --- Inverse NTT Butterfly (Gentleman-Sande) ---
...
"""
    backend_ntt_transform(::MetalBackend, poly, modulus)

Forward NTT using Cooley-Tukey butterflies on Apple Metal GPU.
"""
function ProvenCrypto.backend_ntt_transform(backend::ProvenCrypto.MetalBackend, poly::AbstractVector, modulus::Integer)
    ...
    d_data = MtlArray{Int32}(Int32.(poly))
    ...
    d_zetas = MtlArray{Int32}(zetas_padded)

    ka_backend = MetalBackend()
    half_n = n ÷ 2
...
function ProvenCrypto.backend_ntt_transform(backend::ProvenCrypto.MetalBackend, mat::AbstractMatrix, modulus::Integer)
    result = similar(mat)
    for i in axes(mat, 1)
...
"""
    backend_ntt_inverse_transform(::MetalBackend, poly, modulus)

Inverse NTT using Gentleman-Sande butterflies on Apple Metal GPU.
"""
function ProvenCrypto.backend_ntt_inverse_transform(backend::ProvenCrypto.MetalBackend, poly::AbstractVector, modulus::Integer)
    n = length(poly)
    ...
    d_zetas_inv = MtlArray{Int32}(zetas_inv_padded)
...
function ProvenCrypto.backend_ntt_inverse_transform(backend::ProvenCrypto.MetalBackend, mat::AbstractMatrix, modulus::Integer)
    result = similar(mat)
    for i in axes(mat, 1)
...
"""
    backend_lattice_multiply(::MetalBackend, A, x)

Matrix multiplication on Apple Metal GPU.
"""
function ProvenCrypto.backend_lattice_multiply(backend::ProvenCrypto.MetalBackend, A::AbstractMatrix, x::AbstractVector)
    d_A = MtlArray{Float32}(Float32.(A))
    d_x = MtlArray{Float32}(Float32.(x))
    d_result = d_A * d_x
    return round.(Int, Array(d_result))
end

function ProvenCrypto.backend_lattice_multiply(backend::ProvenCrypto.MetalBackend, A::AbstractMatrix, B::AbstractMatrix)
    d_A = MtlArray{Float32}(Float32.(A))
    d_B = MtlArray{Float32}(Float32.(B))
    d_result = d_A * d_B
    return round.(Int, Array(d_result))
end

"""
    backend_polynomial_multiply(::MetalBackend, a, b, modulus)

NTT-based polynomial multiplication on Apple Metal GPU.
"""
function ProvenCrypto.backend_polynomial_multiply(backend::ProvenCrypto.MetalBackend, a::AbstractVector, b::AbstractVector, modulus::Integer)
    ...
    d_a = MtlArray{Int32}(Int32.(a_ntt))
    d_b = MtlArray{Int32}(Int32.(b_ntt))
    d_c = (d_a .* d_b) .% q
    c_ntt = Array(d_c)
...
"""
    backend_sampling(::MetalBackend, distribution, params...)

GPU-parallel CBD sampling on Apple Metal.
"""
function ProvenCrypto.backend_sampling(backend::ProvenCrypto.MetalBackend, distribution::Symbol, params...)
    if distribution == :cbd
        ...
        d_random = MtlArray{Int32}(Int32.(random_bytes))
        d_output = Metal.zeros(Int32, total_samples)
...
end # module ProvenCryptoMetalExt

...
`+ffi/zig/+` is now concrete (non-template) and exports concrete
`+axiom_*+` symbols:
...
For release/readiness decisions, treat the Zig FFI path as
authoritative. Treat Idris2 ABI files as formal scaffold/specification
that now typechecks and uses concrete Axiom naming.

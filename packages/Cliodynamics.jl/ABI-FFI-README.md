...
# Cliodynamics ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcliodynamics.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── cliodynamics.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cliodynamics.h       # Generated from Idris2 ABI
│
...
public export
simulationResultSize : HasSize SimulationResult 24

-- Prove field alignment is correct
public export
fieldAligned : Divides 8 (offsetOf SimulationResult.time)
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cliodynamics.h
```
...
```c
#include "cliodynamics.h"

int main() {
    void* handle = cliodynamics_init();
    if (!handle) return 1;

    int result = cliodynamics_process(handle, 42);
    if (result != 0) {
        const char* err = cliodynamics_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcliodynamics -L./zig-out/lib
```

### From Idris2

```idris
import Cliodynamics.ABI.Foreign
...
```rust
#[link(name = "cliodynamics")]
extern "C" {
    fn cliodynamics_init() -> *mut std::ffi::c_void;
    fn cliodynamics_free(handle: *mut std::ffi::c_void);
    fn cliodynamics_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cliodynamics_init();
        assert!(!handle.is_null());

        let result = cliodynamics_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcliodynamics = "libcliodynamics"

function init()
    handle = ccall((:cliodynamics_init, libcliodynamics), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cliodynamics_process, libcliodynamics), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cliodynamics_free, libcliodynamics), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cliodynamics.h
   ```
